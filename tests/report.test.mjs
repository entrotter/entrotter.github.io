...
const evm=()=>JSON.parse(readFileSync(new URL('../reports/ethereum-uniswap-slippage.json',import.meta.url),'utf8'));
test('real archived-state artifact passes hash and receipt view validation',async()=>{
  const r=evm();await context.checkHash(r);
  ...
  assert.equal(view.rows.find(r=>r[0]==='USDC final token units')[1],'2556.134769');
});
test('token units preserve integers beyond Number safe precision',()=>{
  assert.equal(context.tokenUnits('123456789012345678901',18),'123.456789012345678901');
